...
abstract class Store<Key, Output> {
  ...
  Stream<StoreResponse<Output>> stream(StoreRequest<Key> request);
...
extension A<Key, Output> on Store<Key, Output> {
// /**
...
// }.first().requireData()
  ...
  Stream<Output> get(Key key) => this
      .stream(StoreRequest.cached(key, false))
      .where((event) => !(event is Loading || event is NoNewData))
...
// }.first().requireData()
  ...
  Stream<Output> fresh(Key key) => this
