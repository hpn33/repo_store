...
import 'impl/real_source_of_truth.dart';
...
abstract class SourceOfTruth<Key, Input, Output> {
  ...
  static SourceOfTruth<Key, Input, Output> of<Key, Input, Output>({
    Stream<Output?> Function(Key)? reader,
    Future<Output?> Function(Key)? nonStreamReader,
    required Future<void> Function(Key, Input) writer,
    Future<void> Function(Key)? delete,
    Future<void> Function()? deleteAll,
  }) =>
      reader == null
          ? PersistentNonFlowingSourceOfTruth(
              realReader: nonStreamReader!,
              realWriter: writer,
              realDelete: delete,
  ...
  // @JvmName("ofFlow")
  // static SourceOfTruth<Key, Input, Output> of<Key, Input, Output>(
  //   Future<Output?> Function(Key) nonFlowReader,
  //   Future<void> Function(Key, Input) writer,
  //   Future<void> Function(Key)? delete,
  //   Future<void> Function()? deleteAll,
  // ) =>
  //     PersistentSourceOfTruth(
...
//      */
class WriteException extends Excp
//  RuntimeException(
...
// )
{
  ...
  WriteException(
      /**
         ...
         */
      this.key, // TODO why are we not marking keys non-null ?
      /**
      ...
      /**
         ...
         */
      this.cause)
      : super("Failed to write value to Source of Truth. key: $key", cause);
...
//  * see [StoreResponse.Error.Exception]
//  */
class ReadException extends Excp {
  final Object? key;
  final String cause;
  ...
  ReadException(
      /**
         * The key for the failed write attempt
         */
      this.key, // TODO shouldn't key be non-null?
      this.cause)
      : super(
