...
// @ExperimentalCoroutinesApi
// @FlowPreview
import 'package:repo_store/src/store_response.dart';
...
class RealStore<Key, Input , Output > extends Store<Key, Output> {
...
// TODO
  ...
  //   _sourceOfTruth = SourceOfTruthWithBarrier(sourceOfTruth);
  // }


        // TODO
//         if (memoryPolicy != null) {
// // _memoryPolicy =         CacheBuilder.newBuilder().apply {
//           final mp = CacheBuilder.newBuilder();

//             if (memoryPolicy.hasAccessPolicy) {
//                 mp.expireAfterAccess(
//                     memoryPolicy.expireAfterAccess.inWholeMilliseconds,
//                     TimeUnit.MILLISECONDS
//                 );
//             }
...
//             if (memoryPolicy.hasMaxSize) {
//             mp.    maximumSize(memoryPolicy.maxSize);
//             }

//             if (memoryPolicy.hasMaxWeight) {
//                 mp.maximumWeight(memoryPolicy.maxWeight);
//                 mp.weigher((k, v) => memoryPolicy.weigher.weigh(k, v));
//                 //  { k: Key, v: Output -> memoryPolicy.weigher.weigh(k, v) }
//             }
//         // }
//       _memCache =  mp..build<Key, Output>();

//         }

// TODO
// _fetcherController = FetcherController(
//         // scope = scope,
//         realFetcher: fetcher,
...
final _memCache;
    ...
    //  * requests are shared.
    //  */
    final  FetcherController _fetcherController ;
    ...
    Stream<StoreResponse<Output>> stream<Key>(StoreRequest<Key> request ) =>
        flow<StoreResponse<Output>> {
            final cachedToEmit = if (request.shouldSkipCache(CacheType.memory)) {
                null
            } else {
                _memCache?.getIfPresent(request.key)
            }
    ...
    @override Future<void> clear(Key key) {
        _memCache?.invalidate(key);
        _sourceOfTruth?.delete(key);
    }
    // override suspend fun clear(key: Key) {
    //     memCache?.invalidate(key)
    //     sourceOfTruth?.delete(key)
    ...
    @override Future<void> clearAll() {
        _memCache?.invalidateAll();
        _sourceOfTruth?.deleteAll();
    }
    ...
    //  * This ensures we first get the value from disk and then load from server if necessary.
    //  */
    Stream<StoreResponse<Output>>  _diskNetworkCombined(
        StoreRequest<Key> request,
        SourceOfTruthWithBarrier<Key, Input, Output> sourceOfTruth
    ){
        final diskLock = CompletableDeferred<Unit>();
        final networkLock = CompletableDeferred<Unit>();
        final networkFlow = createNetworkFlow(request, networkLock);
        final skipDiskCache = request.shouldSkipCache(CacheType.DISK);
        if (!skipDiskCache) {
            diskLock.complete(Unit);
        }
        final diskFlow = sourceOfTruth.reader(request.key, diskLock).onStart {
            // wait for disk to latch first to ensure it happens before network triggers.
            // after that, if we'll not read from disk, then allow network to continue
            if (skipDiskCache) {
                networkLock.complete(Unit);
        ...
        // we use a merge implementation that gives the source of the flow so that we can decide
        // based on that.
        return networkFlow.merge(diskFlow)
        .transform {
        //     // left is Fetcher while right is source of truth
        //     when (it) {
        //         is Either.Left -> {
    ...
    Stream<StoreResponse<Input>> createNetworkFlow(
        {StoreRequest<Key> request ,
CompletableDeferred<Unit>?        networkLock ,
        bool piggybackOnly = false,}
    ) {
        return _fetcherController
            .getFetcher(request.key, piggybackOnly)
            .onStart( () {
                // wait until disk gives us the go
                networkLock?.await();
