...
class FetcherController<Key , Input , Output >{
  ...
  FetcherController(
    ...
    // @Suppress("USELESS_CAST") // needed for multicaster source
    final fetchers = RefCountedResource(
        create:  (Key key) {
            // return Multicaster(
            //     scope = scope,
            //     bufferSize = 0,
        ...
        onRelease: (  Key _,  Multicaster<StoreResponse<Input>> multicaster) =>
            multicaster.close()
    ...
    //     }
    // }
    Stream<StoreResponse<Input>> getFetcher(Key key , [bool piggybackOnly = false]) {
        return flow {
            val fetcher = acquireFetcher(key)
            try {
                emitAll(fetcher.newDownstream(piggybackOnly))
    ...
    //  * the operation).
    //  */
    Future<void> _acquireFetcher(Key key) {
    ...
    // visible for testing
    @protected
    Future<void> fetcherSize() => fetchers.size();
